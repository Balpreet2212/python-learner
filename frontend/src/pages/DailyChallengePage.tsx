...
import { getDailyChallenge, submitDaily, type DailyChallenge } from "../api/daily";
...
export default function DailyChallengePage() {
  ...
  async function handleSubmit() {
    ...
    setSubmitError(null);
    try {
      const res = await submitDaily(code);
      setResult(res);
        ...
        <section className="space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              <span className={`text-xs font-semibold uppercase tracking-wider ${style.muted}`}>
                Daily Challenge · {challenge.date_key}
              </span>
            ...
            <div className="flex shrink-0 flex-col items-end gap-1.5">
              ...
              <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${DIFFICULTY_BADGE[challenge.difficulty] ?? DIFFICULTY_BADGE["easy"]}`}>
          ...
          {challenge.already_passed && !result?.all_passed && (
            <div className="rounded-xl border border-green-700/40 bg-green-950/20 px-4 py-3">
              <p className="text-sm text-green-400">You already completed today's challenge! Come back tomorrow for a new one.</p>
            </div>
          )}
        ...
        <section className="space-y-4">
          ...
          {passed && (
            <div className="rounded-xl border border-green-700/40 bg-green-950/20 px-5 py-4 text-center">
              ...
              <p className={`mt-1 text-sm ${style.muted}`}>Come back tomorrow for a new challenge.</p>
            </div>
          )}
        </section>
